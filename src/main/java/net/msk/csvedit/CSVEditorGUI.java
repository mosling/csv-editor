        ...
        extends javax.swing.JFrame
{
    ...
    private void changeActiveComponents()
    {
        ...
        boolean b = CSVEditor.csvproperties.getTreeVisible();
        jSplitPane
                .getLeftComponent()
                .setVisible( b );
        if ( b )
        ...
        boolean c = CSVEditor.csvproperties.getLargeIcons();
        if ( c )
        {
        ...
        {
            jButtonUngroup.setText( java.util.ResourceBundle
                    .getBundle( "csvlang" )
                    .getString( "ungroup" ) );
            jButtonGroupBy.setText( java.util.ResourceBundle
                    .getBundle( "csvlang" )
                    .getString( "group_by" ) );
            jButtonSortColumn.setText( java.util.ResourceBundle
                    .getBundle( "csvlang" )
                    .getString( "sort" ) );
            jButtonInsertRow.setText( java.util.ResourceBundle
                    .getBundle( "csvlang" )
                    .getString( "insert_row" ) );
            jButtonChangeRow.setText( java.util.ResourceBundle
                    .getBundle( "csvlang" )
                    .getString( "change_row" ) );
            jButtonDelRow.setText( java.util.ResourceBundle
                    .getBundle( "csvlang" )
                    .getString( "delete_row" ) );
        }
    }
    ...
    private void initComponents()
    {
        ...
        treeVisible.setSelected( CSVEditor.csvproperties.getTreeVisible() );
        ...
        largeIcons.setSelected( CSVEditor.csvproperties.getLargeIcons() );
    ...
    {//GEN-FIRST:event_jTreeValueChanged
        if ( !treeAdjust )
        {
            ...
            if ( !selectableJTreeNode( selnode ) )
            {
                ...
                TreePath newChosenPath = new TreePath( ( (DefaultMutableTreeNode) ( newChosenNode ) ).getPath() );
...
//            CSVEditor.csvproperties.store();
                ...
                .getActionCommand()
                .equalsIgnoreCase( "delrow" ) )
        {
            delrow();
        }
        else if ( evt
                .getActionCommand()
                .equalsIgnoreCase( "insertrow" ) )
        {
            insertrow();
        }
        else if ( evt
                .getActionCommand()
                .equalsIgnoreCase( "changerow" ) )
        {
            changerow();
        }
        else if ( evt
        ...
        //      else if (evt.getActionCommand().equalsIgnoreCase("useformatfile")) { useFormatFile(); }
    ...
    private void open()
    {
        ...
        JCheckBox    cb = new JCheckBox( java.util.ResourceBundle
                .getBundle( "csvlang" )
                .getString( "use_formatfile" ), false );
        fc.setAccessory( cb );
        ...
        if ( retval == JFileChooser.APPROVE_OPTION )
        {
            ...
            String fname        = selectedFile
                    .getAbsoluteFile()
                    .toString();
            csvreader.readFile( fname );

            // set window name
            this.setTitle( selectedFile.getName() + " - " + java.util.ResourceBundle
                    .getBundle( "csvlang" )
                    .getString( "CSVEditor" ) );

            String main = java.util.ResourceBundle
                    .getBundle( "csvlang" )
                    .getString( "complete_table" );
            ...
            if ( cb.isSelected() )
                if ( csvreader.isFormatFileAvailable() )
                    ...
                    javax.swing.JOptionPane.showMessageDialog( this, java.util.ResourceBundle
                            .getBundle( "csvlang" )
                            .getString( "Message_FormatFile_Not_Available" ) );
            ...
            CSVEditor.csvproperties.setCurrentFolder( fc
                    .getSelectedFile()
                    .getAbsolutePath() );
        }
    }
    ...
    private void save()
    {
        JFileChooser fc = new JFileChooser( CSVEditor.csvproperties.getCurrentFolder() );
        JCheckBox    cb = new JCheckBox( java.util.ResourceBundle
                .getBundle( "csvlang" )
                .getString( "with_header" ), true );
        fc.setAccessory( cb );
    ...
    private void changerow()
    {
        ...
        // now create an dialog and insert the CSVLine into the CSVTable
        CSVLine      line = new CSVLine();
        CSVLineInput linp = new CSVLineInput( this, csvtable, line, defaults, csvreader.isFormatFileUsed(), true );
        linp.setVisible( true );
    ...
    private void ungroup()
    {
        ...
        try
        {
            selnode = (DefaultMutableTreeNode) node
                    .getParent()
                    .getParent();
    ...
    private void consolidateJTreeObjectStructureByAddingRow( CSVLine line )
    {
        ...
        for ( Enumeration el = root.preorderEnumeration(); el.hasMoreElements(); )
        {
            ...
            if ( selectableJTreeNode( node ) )
            {
                ...
                while ( itTable.hasNext() )
                {
                    ...
                    if ( tableEntry
                            .toString()
                            .equals( lineEntry.toString() ) )
                        continue;
                    // sonst sind die Anforderungen, um in die Tabelle aufgenommen zu werden nicht erfuellt
                    hiddencolsMatch = false;
                    break;
                }
                if ( hiddencolsMatch )
        ...
        for ( Enumeration el = root.preorderEnumeration(); el.hasMoreElements(); )
        {
            ...
            if ( node.isLeaf() )
                continue;
            // wenn die line nicht in die Tabelle des Knotens aufgenommen wurde, braucht er nicht betrachtet zu werden
            if ( !( (CSVTable) ( node.getUserObject() ) )
                    .getData()
                    .contains( line ) )
                continue;
            ...
            for ( Enumeration eg = node.children(); eg.hasMoreElements(); )
            {
                ...
                for ( Enumeration ec = groupnode.children(); ec.hasMoreElements(); )
                {
                    ...
                    if ( ( (CSVTable) ( child.getUserObject() ) )
                            .getData()
                            .contains( line ) )
                    {
                ...
                if ( !isInExistingChild )
                {
                    ...
                    int i = 0;
                    while ( ( i < groupnode.getChildCount() ) && ( 0 > groupnode
                            .getChildAt( i )
                            .toString()
                            .compareTo( tmp ) ) )
                    {
    ...
    private void delrow()
    {
        ...
        for ( int selrowscount = jTable.getSelectedRowCount(); selrowscount > 0; selrowscount-- )
        {
                ...
                for ( Enumeration el = root.preorderEnumeration(); el.hasMoreElements(); )
                {
                    ...
                    if ( selectableJTreeNode( node ) )
                    {
                        ...
                        if ( ( rows == 0 ) && ( node != root ) )
                        {
                            ...
                            {
                                if ( node == selnode )
                                    selnode = (DefaultMutableTreeNode) ( ( selnode.getNextSibling() != null ) ?
                                            selnode.getNextSibling() :
                                            selnode.getPreviousSibling() );
            ...
            // FALLS NUN SELBST LEERE TABELLE angezeigt => ANSICHT AUF ANDERE TABELLE (selnode) WECHSELN
            if ( jTable.getRowCount() == 0 )
            {
                try
                {
                    TreePath           selPath     = new TreePath( selnode.getPath() );
                    TreePath           oldLeadPath = jTree.getLeadSelectionPath();
                    TreeSelectionEvent ev          = new TreeSelectionEvent( jTree, selPath, false, oldLeadPath,
                            selPath );
                    jTreeValueChanged( ev );
                ...
                {
                    LOGGER.debug( "Knoten " + selnode.toString()
                            + " unerwartet ausgewdhlt, setze aktuellen Knoten auf root" );
                    selnode = root;
                    TreePath           selPath     = new TreePath( selnode.getPath() );
                    TreePath           oldLeadPath = jTree.getLeadSelectionPath();
                    TreeSelectionEvent ev          = new TreeSelectionEvent( jTree, selPath, false, oldLeadPath,
    ...
    private void groupby()
    {
        ...
        {
            LOGGER.debug( "Vor dem Gruppieren Spalte auswdhlen!" );
            return;
        }
        ...
        * an entry at the tree */
        ...
        DefaultMutableTreeNode groupnode = new DefaultMutableTreeNode( new String( csvtable.getColumnName( col ) ) );
        selnode.add( groupnode );

        // from now we need the real column
        currentTable.sort( col );

        String                 tmp    = new String( "something unpossible data: -=-" );
        String                 tmpvgl = null;
        CSVLine                ltmp   = null;
        CSVTable               tab    = null;
    ...
    {
        Enumeration ecolNames = csvtable
                .getReader()
                .getHeader()
                .elements();
        Enumeration ecol      = jTable
                .getTableHeader()
                .getColumnModel()
                .getColumns();
